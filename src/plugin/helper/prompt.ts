...
export const showPromptModal = (title: string, md: string, hasCache: boolean): Promise<QuestionReactive> => new Promise<QuestionReactive>(resolve => {
  ...
  const prompt = computed(() => `参考文件内容: \n${md}\n\n问题: ${question.value}\n\n参数表中<span class="extend $1">$2</span>指的是: $2参数结构是以<!-- div:extend-table $1-table -->开头并且以<!-- extend:end -->结尾的表格内容\n\n特别注意，主结构体放到最上面，主结构体的名称用${title}的英文翻译，当字段说明中有jsonArray字符串、或字段名包含List时字段值是数组，不要漏了任何结构体和字段！！所有的注释：中文标点都转换成英文标点，并且修改下格式将格式调整的更加易读、符合规范如果有标点不符合正常规范也调整下；当参数有必填列的时候，注释需要以必填值为开头，例如：[Y] / [N] / [C]；字段注释和字段放在同一行；如果说明中包含网址，请携带上放到对应的字段注释中，格式例如：参考[xxx](https://xxx)`)
